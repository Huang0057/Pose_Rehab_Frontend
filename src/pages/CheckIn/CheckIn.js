...
const CheckIn = () => {
  ...
  const [month, setMonth] = useState(moment().month() + 1); // 月份从1开始
  const [selectedYear, setSelectedYear] = useState(moment().year());
  const [selectedMonth, setSelectedMonth] = useState(moment().month() + 1); // 月份从1开始

  const today = moment([selectedYear, selectedMonth - 1]); // 月份从0开始
  const startOfMonth = today.clone().startOf("month");
  const endOfMonth = today.clone().endOf("month");

  const weeks = [];
  const days = [];
  const day = startOfMonth.clone().startOf("week"); // 开始于星期日
